...
const props = defineProps({
  ...
  /** number > 0, 0 = GO, or null */
  countdown: { type: [Number, String], default: null },
...
const isGo = computed(() => props.countdown === 0 || props.countdown === 'GO!' || props.countdown === '¡SALIDA!');
...
const strokeDashArrayVal = computed(() => {
  if (isGo.value) return '283 283';
  const raw = typeof props.countdown === 'number' ? props.countdown : 3;
  const val = ((4 - raw) / 3) * 283;
  return `${val} 283`;
});
</script>
  ...
  <Teleport to="body">
    <Transition name="zoom-fade">
      <div
        ...
        :class="{ 'overlay--go': isGo }"
        @click="emit('dismiss')"
      >
        <div class="overlay-grid-bg"></div>
        <div class="overlay-slanted-wipe" :class="{ 'slanted-wipe--go': isGo }"></div>

        <div class="overlay-content">
          <div class="glow-pulse-ring" :class="{ 'pulse-ring--go': isGo }">
            <svg class="ring-svg" viewBox="0 0 100 100">
              <circle class="ring-track" cx="50" cy="50" r="45" />
              <circle
                class="ring-active-bar"
                :class="{ 'ring-active-bar--go': isGo }"
                cx="50"
                cy="50"
                r="45"
            ...
            <div class="center-number-display">
              <div
                ...
                :class="{ 'number--tick': !isGo, 'number--go': isGo }"
              >
                {{ countdownText }}
              </div>
            </div>
          ...
          <div class="telemetry-card" :class="{ 'card--go': isGo }">
            <div class="hazard-stripe-decor" :class="{ 'decor--go': isGo }"></div>
            <div class="card-inner">
              <div class="status-msg font-accent">
                {{ isGo ? '¡SALIDA INMEDIATA!' : 'PRESURIZANDO MOTORES' }}
              </div>
              <div v-if="categoryName" class="category-line font-podium">
                {{ categoryName }}
                <span v-if="phaseLabel" class="phase-tag font-accent">{{ phaseLabel }}</span>
              </div>
              <div class="telemetry-log font-symbols">
                <span class="pulse-indicator" :class="{ 'indicator--go': isGo }"></span>
                <span>SYSTEM STATUS: {{ isGo ? 'LAUNCH OK' : 'STAGE READY' }}</span>
              </div>
            </div>
...
.giant-plate-number {
  font-family: var(--font-accent), Impact, sans-serif;
  font-weight: 950;
  line-height: 1;
  font-style: italic;
  letter-spacing: -2px;
...
.card--go {
  ...
  box-shadow: 0 15px 35px rgba(0, 0, 0, 0.7), 0 0 20px rgba(16, 185, 129, 0.12);
}
