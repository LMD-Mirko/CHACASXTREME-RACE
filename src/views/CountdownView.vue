...
const animateEntrance = () => {
  ...
  tl.to('.impact-overlay', { opacity: 0, duration: 1, ease: "power4.out" })
    ...
    .to('.title-char', { duration: 0.8, opacity: 1, y: 0, filter: 'blur(0px)', stagger: 0.02, ease: "back.out(2)", onComplete: () => {
        document.querySelector('.title').classList.add('glitch-active');
    }}, 0.6)
    ...
    <div class="content main-layout">
      ...
      <div class="hero-logo">
        <div class="logo-wrapper">
          ...
          <div class="edition-badge">IV EDICIÓN</div>
        </div>
      </div>
      ...
      <div class="headline">
        <h1 class="title">
          <div class="title-row">
            <span v-for="(char, i) in 'PREPÁRATE PARA'.split('')" :key="i" class="title-char">{{ char === ' ' ? '&nbsp;' : char }}</span>
          </div>
          <div class="title-row title-bottom">
            <span class="lo-text title-char">LO</span>
            <span class="extreme-text">
               <span v-for="(char, i) in 'EXTREMO'.split('')" :key="i" class="title-char">{{ char }}</span>
            </span>
          </div>
        </h1>
        <p class="subtitle">ESTO NO ES UNA CARRERA, ES UNA BATALLA CONTRA LA MONTAÑA</p>
      </div>

      <!-- Aggressive Countdown -->
      <div class="timer-container">
        <div class="timer-item" v-for="(val, unit) in { DÍAS: timeLeft.days, HORAS: timeLeft.hours, MINUTOS: timeLeft.minutes, SEGUNDOS: timeLeft.seconds }" :key="unit">
      ...
      <div class="social-footer">
        <a href="#" target="_blank" class="social-link"><Instagram :size="32" /></a>
        <a href="#" target="_blank" class="social-link"><Facebook :size="32" /></a>
